...
3. **Place Order** — submit the staged order to SoftPro (required to actually send it)
...
After successfully staging an order, you **must** place it to actually submit the order to SoftPro.
...
* Placing is the step that triggers transmission to the destination system (**SoftPro**).
